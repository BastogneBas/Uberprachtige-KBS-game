...
void Level::setObjectAt(uint8_t x, uint8_t y, uint8_t object, uint8_t drawn = false){
	if(!drawn)
...
void Level::drawMap(){
	...
	Definitions::tft->startWrite();
		for(uint8_t x=0;x<=Definitions::gameWidth+1;x++){
			uint8_t currentObject = getObjectAt(x,y) & ~(mapObject::needsRedraw);
			...
			switch(currentObject){
				case mapObject::air:
					Definitions::tft->writeFillRect(_x, _y, 16, 16, ILI9341_BLACK);
					break;
				case mapObject::block:
					for(uint16_t j=0;j<_h;j++,_y++){
						for(uint16_t i=0;i<_w;i++){
							Definitions::tft->writePixel(_x+i, _y, pgm_read_word(&blokje[j*_w+i]));
						}
					}
					break;
				case mapObject::barrel:
					for(uint16_t j=0;j<_h;j++,_y++){
						for(uint16_t i=0;i<_w;i++){
							Definitions::tft->writePixel(_x+i, _y, pgm_read_word(&ton[j*_w+i]));
						}
					}
					break;
				case mapObject::peep1:
					for(uint16_t j=0;j<_h;j++,_y++){
						for(uint16_t i=0;i<_w;i++){
							Definitions::tft->writePixel(_x+i, _y, pgm_read_word(&peep1[j*_w+i]));
						}
					}
					break;
				case mapObject::peep2:
					for(uint16_t j=0;j<_h;j++,_y++){
						for(uint16_t i=0;i<_w;i++){
							Definitions::tft->writePixel(_x+i, _y, pgm_read_word(&peep2[j*_w+i]));
						}
					}
					break;
